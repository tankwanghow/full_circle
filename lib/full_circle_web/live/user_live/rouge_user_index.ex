defmodule FullCircleWeb.UserLive.RougeUserIndex do
  ...
  def handle_event("delete_user", %{"user_id" => id}, socket) do
    ...
    case FullCircle.Sys.delete_rouge_user(
      ...
      {:ok, _user} ->
        {:noreply,
         socket
         |> push_navigate(to: ~p"/companies/#{socket.assigns.current_company.id}/rouge_users")
  ...
  defp get_user_list(socket) do
    ...
    |> assign(
      :users,
      Sys.get_rouge_users(socket.assigns.current_company, socket.assigns.current_user)
    )
  end
end
