defmodule FullCircleWeb.StatutoryFileFormatLive.Index do
  use FullCircleWeb, :live_view

  alias FullCircle.StatutoryConfig
  alias FullCircleWeb.StatutoryFileFormatLive.IndexComponent
  ...
  def mount(_params, _session, socket) do
    if FullCircle.Authorization.can?(
      ...
      {:ok,
       socket
       |> assign(page_title: gettext("Statutory File Format Listing"))
       |> assign(search: %{terms: ""})
       |> assign_objects("")}
    else
      {:ok,
  ...
  def handle_event("search", %{"search" => %{"terms" => terms}}, socket) do
    ...
    url =
      "/companies/#{socket.assigns.current_company.id}/statutory_file_formats?#{URI.encode_query(qry)}"
  ...
  defp assign_objects(socket, terms) do
    ...
    objects =
      StatutoryConfig.list_versions(:file_format, com_id)
      |> Enum.filter(fn f ->
        terms == "" or
          String.contains?(String.downcase(f.code), String.downcase(terms)) or
          String.contains?(String.downcase(f.name), String.downcase(terms))
      end)
  ...
  def render(assigns) do
    ...
    <div class="w-6/12 mx-auto">
      ...
      <div class="text-center mb-2 flex justify-center gap-2">
        <.link
          navigate={~p"/companies/#{@current_company.id}/statutory_file_formats/new"}
          class="blue button"
          id="new_object"
        >
          {gettext("New Version")}
