defmodule FullCircle.HR do
  ...
  def delete_salary_note_multi(multi, salary_note, com, user, name \\ :delete_salary_note) do
    multi
    # Deliberately NOT StdInterface.changeset/4: that runs the edit-time rules
    # (the ±31/+14 day note_date window, pay-slip lock), and Ecto refuses a
    # Multi.delete whose changeset is invalid. Statutory lines generated by
    # PaySlipOp are dated end_of_month, so validating here makes them
    # undeletable for most of the month and fails the whole pay/6 run.
    |> Multi.delete(name, Ecto.Changeset.change(salary_note))
    |> Multi.delete_all(
