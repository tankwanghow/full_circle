...
> ⚠️ **AR/AP outstanding = the contact-bearing line ONLY.** A financial document (Invoice, PurInvoice, Receipt, …) posts a receivable/payable line with `contact_id` **set** *plus* contra revenue/cost/tax lines with `contact_id IS NULL` — and all lines share the same `doc_type`/`doc_id`. **Summing all of a document's GL lines nets to ZERO** (it's a balanced journal). To compute "amount still owed", filter `t.contact_id IS NOT NULL` (the control-account line) and net it against `transaction_matchers` + `seed_transaction_matchers` (matchers attach to a transaction via `transaction_id`; `match_amount` reduces the open balance). Omitting the `contact_id` filter silently overstates AR/AP by a large factor — a real bug shipped this way (182M reported vs 7.3M true). **Always sanity-check a new AR/AP aggregate against a trusted reference** (`Reporting.debtors_balance/3`, `debtor_aging_report/3`, `creditor_aging_report/3`) for a real company before trusting it — an order-of-magnitude gap means contra lines are leaking in or matchers are missing.
