...
object PunchUploader {
    enum class Result { OK, RETRY, REVOKED }

    fun drainBlocking(context: Context): Result = runBlocking {
        QueueDb.mutex.withLock { drainLocked(context.applicationContext) }
    }

    private suspend fun drainLocked(context: Context): Result {
        val prefs = Prefs(context)
        val token = prefs.token
        val baseUrl = prefs.baseUrl.trimEnd('/')
        if (token.isEmpty() || baseUrl.isEmpty()) return Result.OK

        val dao = QueueDb.get(context).punchDao()
        val rows = dao.all()
        ...
        for (row in rows) {
            when (val outcome = upload(baseUrl, token, row)) {
                ...
                Outcome.Revoked -> {
                    QueueDb.get(context).punchDao().deleteAll()
                    QueueDb.photosDir(context).listFiles()?.forEach { it.delete() }
                    prefs.clear()
        ...
        return if (networkFail) Result.RETRY else Result.OK
    }
    ...
    private fun upload(baseUrl: String, token: String, row: PunchEntity): Outcome {
        ...
        val body = MultipartBody.Builder()
            ...
            .addFormDataPart("photo", "face.jpg", photo.asRequestBody(JPEG))
            .build()
