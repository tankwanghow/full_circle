...
- Screen stays on (`FLAG_KEEP_SCREEN_ON` + `android:keepScreenOn` on the root view) so a tap still works when the phone is in a wall mount (power button unreachable). A real `PowerManager` sleep would ignore taps.
- After **N seconds with no face** (default 20, range 5–120), the camera is unbound and the panel dims. **Link icon top-left** (green = Full Circle reachable). A green icon drains the offline queue in-process; do not wait on WorkManager. Long-press the version (top-right) to change N and see how many punches are not yet sent. Bottom: face/QR examples (**green = ok, red = missing**), then the clock. **Tap to Scan** only while asleep.
...
- Walk away for 20s → screen goes black with “Tap, then show your badge”; tap → preview returns and a punch still works. A second person in a queue should not need to tap.
