defmodule FullCircle.Repo.Migrations.CreateQueries do
  ...
  def up do
    ...
    # execute "DO $do$ BEGIN IF EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'full_circle_query') THEN RAISE NOTICE 'Skipping creation of ROLE full_circle_query'; ELSE CREATE ROLE full_circle_query NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT LOGIN NOREPLICATION NOBYPASSRLS PASSWORD 'nyhlisted'; END IF; END; $do$;"
